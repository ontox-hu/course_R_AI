...
To further demonstrate `mutate()` we will start working with dataset *ce11_gene_exp** which is saved as an R object in *data/lesson5**. This dataset contains gene expression data of all genes in C elegans (genome version 11). The dataset contains six variables:
...
###### Example
```{r}
# import R object from server into Rstudio
ce11_gene_exp <- readRDS("data/lesson5/ce11_gene_exp.rds")     
...
In server folder data/lesson5 is a zipped file named **mart_export.txt.gz** . This file is tab separated file and contains ensemble genes of C. elegans and associated [gene ontology](http://geneontology.org/) descriptions. First we have to import this file into the Rstudio environment and inspect its contents.

###### Example
```{r }

ce11_biomart <- read_tsv(file = "data/lesson5/mart_export.txt.gz") #read_function() can directly import zipped files!
dim(ce11_biomart)
...
<div class="question">
##### Exercise `r les` {-} 
A researcher is using a luciferase reporter assay to study the effect of 8 chemicals in 4 different concentration (in triplo) on the activity of the reporter gene. The data is listed in Excel file **screening.xlsx** in the folder data/lesson5. The file contains two data sheets. One sheet contains the experimental set-up. The second sheet contains the actual data in a 96 wells plate layout
...
excel_sheets("data/lesson5/screening.xlsx")
screening_experiment <- read_excel("data/lesson5/screening.xlsx", sheet="experiment")
screening_data <- read_excel("data/lesson5/screening.xlsx", sheet="data")
