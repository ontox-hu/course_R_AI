# Lesson 2 - importing data and visualisations
...
smoking2_tidy <-  pivot_longer(data = smoking2, cols = c(control, patch),  
...
Or you could make a density plot (note the "y = ..density.."), including some minimal effort into design:

```{r}
# Change line colors by groups

ggplot(data = smoking2_tidy, aes(x = swimming_time, color = group, fill=group)) + 
...
stat="identity" makes sure the bars are next to each other instead of on top of each other.
...
colour_rabbits_summary %>%
  ...
  labs(title = "Figure X. average ear length of rabbits", 
       ...
       x="rabbit colour", 
       y="average ear length (cm)") + 
  ylim(0, 35) + # limits on the y axis
  theme_classic() + #because I don't like grey backgrounds
  ...
  annotate("text", x = 1.5, y = 35, label = paste("two sample t-test p=",p_to_plot))
...
For many specific visualisation requirements there are R packages available that will do most of the work for you. 

Suppose for instance that we have a dataset with expression data for 5 genes in 4 samples and we would like to make a heatmap. For the sake of clarity, we will take a very blunt approach to genetics.
Here are the volunteers who supplied us with the samples:

```{r, echo=FALSE,out.width="15%", out.height="20%",fig.cap="",fig.show='hold',fig.align='center'}
knitr::include_graphics(c(here::here('images','squirtle.jpg'),
                          here::here('images','eevee.jpg'),
                          here::here('images','charizard.jpg'),
                          here::here('images','rattata.png')))
```
...
```{r}
gene_more_hairy <- c(200,400,200,400) 
gene_less_hairy <- c(400,200,400,200) 
gene_big_earsize <- c(2,20,2,20) 
gene_small_earsize <- c(20,2,20,2) 
gene_wingsize <- c(0,0,150,0)

expressiondata <- rbind(gene_more_hairy, gene_less_hairy,
                        gene_big_earsize, gene_small_earsize, gene_wingsize)
...
The least distant two genes are `big_earsize` and `small_earsize`, so those will be clustered together. We can plot a dendrogram of the distance matrix:

```{r dendogram1}
dist(expressiondata) %>% hclust() %>% plot()
```

We can see that the expression of gene `more_hairy` and `less_hairy` are more similar to each other than to the other genes, `big_earsize` and `small_earsize` are similar, and the expression of `wingsize` looks a bit more like that of the earsize genes than to that of the hairy genes. Looking at the data in expression data, that seems reasonable.

Now let's make a heatmap. We will use the **pheatmap** package, and as with the dendogram, it will take just one line of code (well, two if you include loading the package):

```{r, eval=F}
install.packages("pheatmap") # you need to install packages once
```
```{r heatmap1}
library(pheatmap) # and load them every time you start a new session
pheatmap(expressiondata, scale="row")
```

`scale = "row" ` means that we asked pheatmap to center and scale the values, it does so by subtracting the mean (centering) and dividing by the standard deviation (scaling), thus calculating Z scores. You can always find out what arguments you can give to a function by typing ?functionname in the console. In this case: `?pheatmap`.

We see that the expression of `more_hairy` and `big_earsize` seems to be correlated, as well as the `less_hairy` and `small_earsize`. Indeed, the more hairy pokemon in our selection tend to have bigger ears and the less hairy pokemon tend to have smaller ears. Also, the expression of the `wingsize` gene seems to be correlated to expression of `less_hairy` and `small_earsize` genes.

### Explore

We will not try to cover all your specific visualisation needs, as they will probably be very different. We hope this lesson gives you enough background in using ggplot to explore the internet, apply what you find and visualise your own data. 

You could start with taking a look at the [r graph gallery](https://r-graph-gallery.com/index.html).

Please join us in the open breakout room for help in visualising your own data, as well as exchanging usefull packages and showing of visualisations.
