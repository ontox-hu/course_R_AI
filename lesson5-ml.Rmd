...
##### Exercise `r les` {-}
 ...
 2. Read the file into R, using the `
...
A much used machine learning algorithm is 

Beause we have class imbalance we use a stratification approach
```{r}
...
Usually, datasets like the QSAR dataset or Tox21, when making them suitable for Machine Learning they are shared as data split in a file containing the features and a file containing the row descriptors. In the case of Tox21 data, the files are downloadable as files containing the train and test splits of the data, and additional row descriptors. Here we will try to enrich the QSAR data with data from the Tox21 dataset, and use that new dataset to train a new (and hopefully better) model. 

 1. In the folder `./data/lesson5/qsar` additional files to the dataset you just downloaded are stored as `ID.txt` and `X.csv`
 2. Open these files in R
 3. To get the headers of `X.csv`, you need to read in the file `colnames_X.txt`. 
 4. Read this latter file with the `read_csv()` function of `{readr}`. Use the `comment = "##"` argument to skip the lines with the ##. Convert the headers to lowercase.
 5. Re-code the column `toxic` in the X.csv dataset to 'positive' and 'negative'. Use `mutate()` and `ifelse()`. Name the new variable `class`
 6. Move the `class` column to the first position in the dataframe
 7. Join the data of the features (`X.csv` -> fingerprints) and the row descriptors (`ID.txt` -> row descriptors) together, starting with the columns of the row descriptors, than the fingerprint columns
 8. What do you think of the level of 'data provenance' of these files? What would you propose to do better, if you were the creator of these files and requested to share them? What assumptions do we need to make about this data, that we are not able to check based on the information provided.
 
**TIPS**
 
 - Use the `janitor::clean_names()` function to create nice (Linux and R safe) headers.
...
data_rows <- read_csv(
  here::here(
    ...
    "qsar",
    "ID.txt"
  ), col_names = data_x_headers$colname) |>
  janitor::clean_names() |>
  mutate(
    class = ifelse(
      toxic == "TRUE", 
...
# join
data_all_qsar <- dplyr::bind_cols(data_rows, data_fp)

```
</details>


## Enrichment with data from Tox21
Here we enrich the data, with data from the Tox21 dataset. We have prepared our QSAR data above during the exercise. The combined QSAR data, we will use here further to train a new (and hopefully improved) prediction model for toxicity.


Get Tox21 dataset from Open Chem (https://mariewelt.github.io/OpenChem/html/installation_instructions.html)

see for chemical fingerprints etc. : https://www.rdkit.org/docs/Install.html#how-to-install-rdkit-with-conda

```{r}
tox21 <- read_csv(  "https://raw.githubusercontent.com/Mariewelt/OpenChem/master/benchmark_datasets/tox21/tox21.csv")

ind <- tox21$smiles %in% data_all_qsar$canonical_qsa_rr
       
tox21[ind,]
             
joined <-  left_join(data_all_qsar, tox21, by = c("canonical_qsa_rr" = "smiles"))                    
...
Steps:
 ...
 1. Create embeddings for tox21
 2. Train new model using XGBoost
 3. Train on each assay as outcome 
 4. Predict target for qsar data. Can we ascribe toxicity to a target?


## Create structural embedding with a Transformer

Creating different embeddings from molecule structure
Thanks to Tom Luechtefeld (Sysrev.com) for providing this code and demo.

https://mariewelt.github.io/OpenChem/html/tutorials/tox21_tutorial.html
https://docs.conda.io/en/latest/miniconda.html
For Apple (M1): https://developer.apple.com/metal/tensorflow-plugin/ (tensorflow)

```{r, eval=FALSE}  
reticulate::use_condaenv("DC")
library(reticulate)
library(tidyverse)

# rdkit <- reticulate::import("rdkit")
transformers <- reticulate::import('transformers') # this is a huggingface.co api
#openchem <- reticulate::import("openchem.data.utils")

smi <- tox21$smiles[1]

hface <- "seyonec/ChemBERTa-zinc-base-v1"
tok   <- transformers$AutoTokenizer$from_pretrained(hface)
bert  <- transformers$AutoModel$from_pretrained(hface)
...
smi <- hmap |> pivot_longer(cols=c("anc","ana")) |> filter(sim>0.95,sim<0.99)
...
## Adding the features to our qsar data
